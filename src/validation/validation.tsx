...
export const signUpSchema = Yup.object().shape({
  ...
  password: Yup.string()
    .matches(
      passwordPattern,
      "password must be at least 8 characters long , at least one uppercase letter, one lowercase letter, one special character, and one number"
    )
    .required(),
...
export const addingSchema = Yup.object().shape({
  ...
  catagory: Yup.string().min(1, "catagory is required").required(),
  ...
  preview: Yup.mixed()
    ...
    .test("fileSize", "File size exceeds the limit", (value: any) => {
      if (!value) {
        return true;
      }
