const Advertisment = () => {
  const addvertisment = [
    ...
    "Create tailored learning paths for team and organization goals and even host",
  ...
  return (
    <>
      <div className="px-2 py-8 text-4xl flex justify-center items-center text-slate-800 text-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4">
        {addvertisment.map((ad, index) => (
          <div key={index} className="max-w-sm mx-auto">
            <div className="text-center h-56 rounded-lg border-2 bg-gray-200 flex items-center justify-center p-4">
              <h2 className="text-xl sm:text-2xl font-serif">{ad}</h2>
            </div>
          </div>
        ))}
      </div>
