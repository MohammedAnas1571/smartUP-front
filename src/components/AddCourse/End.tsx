...
const End = ({ formik, setChange }: FormikDetails) => {
  const { loading } = useSelector((state: RootState) => state.tutor);
  return (
    <TabsContent value="step-3">
      <Card>
        <CardHeader>
          <CardTitle>Add Thumbnail and Preview</CardTitle>
          <CardDescription>
            Please Provide thumbnail and preview about course
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="flex justify-end gap-3">
          <Button onClick={() => setChange("step-2")}>Back</Button>
